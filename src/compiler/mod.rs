...
impl Compiler {
    ...
    fn walk_tree(module: &ExpandedModuleTree, name: String, parent: &Vec<String>, paths: &mut Vec<Vec<String>>) {
    ...
    fn build_module(&self, mut path: Vec<String>) -> CompilerResult<Module> {
        ...
        for path_el in &path {
            let Some(_expanded_module) = expanded_module.modules.get(path_el) else { unreachable!("internal module search failed") };
        ...
        for extern_use in &expanded_module.extern_uses {
            ...
            let Some(fn_type) = extern_use.ret_type.fn_type(&self.llvm_ctx, &args, extern_use.arg_list) else {
                return Err(todo!());
            };

            let fn_value = llvm_module.add_function(&*extern_use.name, fn_type, Some(Linkage::External));

            let None = frame.entries.insert(extern_use.name.clone(), (extern_use.args.clone(), extern_use.ret_type.clone(), extern_use.arg_list, fn_value)) else {
                return Err(CompilerError::EntryAlreadyExistsInFrame(extern_use.name.clone()));
            };
        }
        ...
        for (name, entry) in &expanded_module.entries {
            ...
            let Some(fn_type) = entry.type_sig.ret_type.fn_type(&self.llvm_ctx, &args, false) else {
                return Err(todo!());
            };

            let function = llvm_module.add_function(&name, fn_type, Some(Linkage::Internal));

            let None = frame.entries.insert(name.clone(), (entry.type_sig.args.iter().map(|(_, ty)| ty.clone()).collect(), entry.type_sig.ret_type.clone(), false, function)) else {
                return Err(CompilerError::EntryAlreadyExistsInFrame(name.clone()));
            };

            let mut frame = frame.clone();
            frame.variables.extend(entry.type_sig.args.iter().zip(function.get_param_iter()).map(|((name, ty), val)| {
                (name.clone(), (ty.clone(), Some(val.into())))
            }));
    ...
    fn build_block<'a>(&'a self, frame: &StackFrame<'a>, builder: &'a Builder<'a>, block: &Block, name: String, function: FunctionValue<'a>) -> CompilerResult<()> {
    ...
    fn build_standalone_statement<'a>(&'a self, mut frame: RefMut<'a, StackFrame<'a>>, builder: &'a Builder<'a>, statement: &Statement) -> CompilerResult<()> {
        match statement {
            ...
            crate::ast::Statement::If { condition: _, block: _, else_ifs: _, _else } => todo!(),
    ...
    fn build_let<'a>(&'a self, mut frame: RefMut<'a, StackFrame<'a>>, builder: &'a Builder<'a>, name: Str, ty: Option<Type>, value: Option<Box<Statement>>) -> CompilerResult<()> {
    ...
    fn build_literal(&self, ty: Option<Type>, lit: Literal) -> CompilerResult<(Type, BasicValueEnum)> {
        Ok(match lit {
            Literal::Integer(int) => {
                ...
                (ty.clone(), match ty {
                    Type::Char | Type::I8 => self.llvm_ctx.i8_type().const_int(int, sign_extend),
                    Type::U8 => if sign_extend {
            ...
            Literal::String(s) => (Type::Ptr(Box::new(Type::Char)), self.llvm_ctx.const_string(s.as_bytes(), true).into()),
            _ => todo!(),
        })
    }
    fn build_get<'a>(&'a self, frame: &StackFrame<'a>, ty: Option<Type>, name: Str) -> CompilerResult<(Type, BasicValueEnum<'a>)> {
